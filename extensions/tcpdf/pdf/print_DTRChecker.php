...
$clientsForm = new printDTRReport();
$clientsForm -> getDTRReport();

class printDTRReport{

public function getDTRReport(){
    ...
    $has_data = (new ModelDTR)->mdlGetBranchDailyDTRUploadList($check_entry_date);
    ...
    <table style="width: 100%;">
        ...
        <tr>
            <td style="width: 25%;"></td> <!-- Empty cell for spacing -->
            <td style="width: 55%; text-align: center;"><h1>DTR SUBMISSION REPORT</h1></td> <!-- Centered cell -->
...
$header .= <<<EOF
    ...
    if ($has_data === 'yes') {
        ...
        foreach ($data as &$item) {
            ...
            if (preg_match($pattern, $branch_name)) {
                $branch_replace = preg_replace('/RLC/', 'RFC', $branch_name);
            } else {
                $branch_replace = preg_replace('/RLC/', 'RFC', $branch_name);
            }
            
            //Function to check if branch had been submitted a file or not
            $if_has_upload = (new ControllerDTR)->ctrGetBranchDailyDTRUploadList($branch_name, $check_entry_date);
            
              // Condition if the branch has submitted a file or not
            $answer = ($if_has_upload === 'yes') ? 'DONE' : 'UNDONE';

            $file_info = (new ModelDTR)->mdlGetFileInfo($branch_name, $check_entry_date);
            if (!empty($file_info)) {
            ...
            <table style="border: none;margin-top: 100px;">  
                ...
                <tr  style="border: 1px solid #black;">
                    ...
                    <td style="width:125px;text-align:left;font-size:8px; border: 1px solid #black;">$branch_replace</td>
                                                                        
                    <td style="width:80px;text-align:left;font-size:7px;margin-right: 5px; border: 1px solid #black;"><i>$answer</i></td>

                    <td style="width:80px;text-align:left;font-size:8px;margin-right: 15px; border: 1px solid #black;">$format_entry_date</td>
        ...
        foreach ($data as &$item) {
            ...
            $format_entry_date = date('M d, Y', strtotime($check_entry_date));
